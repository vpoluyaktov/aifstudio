...
import { EditorView, keymap, lineNumbers, highlightSpecialChars, drawSelection, dropCursor, rectangularSelection, crosshairCursor, highlightActiveLine } from "https://esm.sh/@codemirror/view@6";
import { EditorState, Compartment } from "https://esm.sh/@codemirror/state@6";
import { defaultHighlightStyle, syntaxHighlighting, indentOnInput, bracketMatching, foldGutter, foldKeymap, StreamLanguage, HighlightStyle } from "https://esm.sh/@codemirror/language@6";
...
// ── Inform 7 syntax highlighting ─────────────────────────────────────────────

const inform7Language = StreamLanguage.define({
  name: "inform7",
  startState() { return { inComment: false }; },
  copyState(state) { return { inComment: state.inComment }; },
  token(stream, state) {
    // Multi-line comment continuation: still inside [...]
    if (state.inComment) {
      while (!stream.eol()) {
        if (stream.next() === ']') {
    ...
    if (stream.peek() === '[') {
      ...
      var closed = false;
      while (!stream.eol()) {
        if (stream.next() === ']') { closed = true; break; }
      }
      if (!closed) state.inComment = true;
      return "comment";
    }
    ...
    // Section headings at start of line: Volume / Book / Part / Chapter / Section
    if (stream.sol() && stream.match(/^(Volume|Book|Part|Chapter|Section)\b.*$/)) {
      return "keyword";
    }

    // Rule keywords at start of line
    if (stream.sol() && stream.match(/^(Instead of|Before|After|When|Every turn|Carry out|Report|Check|To say|To decide|Understand)\b/i)) {
      return "keyword";
    }

    // Boolean atoms
    if (stream.match(/^(true|false|yes|no)\b/)) {
      return "atom";
    }
...
const inform7HighlightStyle = HighlightStyle.define([
  { tag: tags.keyword, color: '#60d6fa' },
  { tag: tags.string,  color: '#a5d6a7' },
  { tag: tags.comment, color: '#6a737d', fontStyle: 'italic' },
  { tag: tags.number,  color: '#f8c555' },
  { tag: tags.atom,    color: '#c3e88d' },
]);
...
function init() {
  ...
  var sourceContainer = document.getElementById('wsSourceEditor');
  cmEditor = new EditorView({
    state: EditorState.create({
      ...
      extensions: [
        ...
        inform7Language,
        syntaxHighlighting(inform7HighlightStyle),
        readOnlyCompartment.of(EditorState.readOnly.of(true)),
        EditorView.theme({
          '&': { background: '#0a0d14', height: '100%' },
          '.cm-content': { caretColor: '#e6edf3' },
          '&.cm-focused .cm-cursor': { borderLeftColor: '#e6edf3' },
          '.cm-selectionBackground, ::selection': { background: 'rgba(167,139,250,0.25)' },
          '&.cm-focused .cm-selectionBackground': { background: 'rgba(167,139,250,0.25)' },
        }, { dark: true }),
      ]
    }),
  ...
  editToggle.addEventListener('change', function () {
    setEditorReadOnly(!this.checked);
  });
  ...
  window.addEventListener('beforeunload', function (e) {
    if (state !== 'loading' && getEditorContent() !== savedSource) {
      e.preventDefault();
...
function setStreamingHint(on) {
  if (on) {
    ...
    setEditorReadOnly(!editToggle.checked);
  }
}
  ...
  apiFetch('/api/projects/' + encodeURIComponent(projectId) + '/source', {
    ...
    .then(function (r) {
      ...
      if (r.ok || r.status === 204) {
        ...
        setEditorContent(historyPreviewSource);
